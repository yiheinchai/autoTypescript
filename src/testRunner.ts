...
import { transformCodeForInstrumentation, generateRuntimeInstrumentationCode } from './instrumentation';
...
export class TestRunner {
    ...
    constructor(
        ...
        this.setupFilePath = path.join(workspaceRoot, '.autotypescript', 'setup.js');
    }
    ...
    async runTests(testCommand: string): Promise<void> {
        ...
        this.outputChannel.appendLine('[AutoTypeScript] Starting test run with type capture...');
        this.outputChannel.appendLine(`[AutoTypeScript] Test command: ${testCommand}`);
        this.outputChannel.appendLine('='.repeat(50));
        ...
        try {
            ...
            this.outputChannel.appendLine(`[AutoTypeScript] Adjusted command: ${adjustedCommand}`);
            ...
            this.outputChannel.appendLine(`[AutoTypeScript] Type cache updated: ${this.cacheManager.getCacheFilePath()}`);

            const stats = this.cacheManager.getStats();
            this.outputChannel.appendLine(`[AutoTypeScript] Functions tracked: ${stats.functionCount}`);
            this.outputChannel.appendLine(`[AutoTypeScript] Total function calls: ${stats.totalCalls}`);
    ...
    private async createSetupFile(): Promise<void> {
        ...
        const setupCode = generateRuntimeInstrumentationCode(cacheFilePath);

        fs.writeFileSync(this.setupFilePath, setupCode);
        this.outputChannel.appendLine(`[AutoTypeScript] Created setup file: ${this.setupFilePath}`);
    }
    ...
    private adjustTestCommand(testCommand: string): string {
        ...
        if (cmd.includes('node ')) {
            return `node --require "${this.setupFilePath}" ${testCommand.replace(/^node\s+/i, '')}`;
        }
    ...
    private executeTests(command: string): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            const env = {
                ...process.env,
                NODE_OPTIONS: `--require "${this.setupFilePath}" ${process.env.NODE_OPTIONS || ''}`.trim(),
                AUTOTYPESCRIPT_CACHE_PATH: this.cacheManager.getCacheFilePath(),
            };

            this.outputChannel.appendLine(`[AutoTypeScript] Running: ${cmd} ${args.join(' ')}`);
            ...
            this.runningProcess.on('close', (code) => {
                ...
                if (code === 0 || code === null) {
                    ...
                    this.outputChannel.appendLine(`[AutoTypeScript] Tests exited with code ${code}`);
                    resolve();
                }
            });
    ...
    private async loadCapturedData(): Promise<void> {
        ...
        if (fs.existsSync(cacheFilePath)) {
            try {
                ...
                this.outputChannel.appendLine('[AutoTypeScript] Type cache loaded and merged.');
            } catch (error) {
                this.outputChannel.appendLine(`[AutoTypeScript] Failed to load captured data: ${error}`);
            }
        } else {
            this.outputChannel.appendLine('[AutoTypeScript] No type cache file found after test run.');
        }
    }
    ...
    instrumentFile(filePath: string): string | null {
        try {
            ...
            this.outputChannel.appendLine(`[AutoTypeScript] Failed to instrument file ${filePath}: ${error}`);
            return null;
        }
    }
}
